...
func (s *Service) resolveSessionProvider(ctx context.Context, manager *session.Manager, current *session.Session, agentName, requestedProviderID string, allowBind bool) (*session.AgentBinding, *SessionProviderConfig, error) {
	// Session provider isolation is disabled on this delivery line.
	// Keep a no-op resolver so call sites compile; never bind or inject session RuntimeEnv.
	_ = ctx
	_ = manager
	_ = current
	_ = agentName
	_ = requestedProviderID
	_ = allowBind
	return nil, nil, nil
}
